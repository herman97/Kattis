...
/*
    Minimize the length of bridges between islands given by coordinates.

    Solved with minimum spanding tree algorirthm.
*/
...
struct Edge
{
    ...
    Edge(float dist, Node n1, Node n2)
    {
        this->dist = dist;
        this->node1 = n1;
        this->node2 = n2;
...
struct Graph
{
    ...
    void krusses()
    {
        ...
        for (int i = 0; i < this->edges.size(); i++)
        {
            ...
            if (p1 != p2)
            {
                cout << this->edges[i].dist << " dist " << endl;
...
ve create_edges(vn nodes)
{
    ...
    for (int i = 0; i < nodes.size(); i++)
    {
        for (int j = i + 1; j < nodes.size(); j++)
        {
            ...
            float temp_dist = sqrt(pow(nodes[i].x - nodes[j].x, 2) + pow(nodes[i].y - nodes[j].y, 2));
